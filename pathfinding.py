...
class PathFinding:
    """Find valid drone paths while respecting graph capacities."""

    def __init__(self, graph: Graph) -> None:
        """Initialize the pathfinding system.

        Args:
            graph: Graph containing zones, connections, and drone data.
    ...
    def find_path(self) -> List[Tuple[Zone, int]]:
        """Find a valid path from the start hub to the end hub.

        The search considers movement cost, heuristic distance,
        zone capacity, connection capacity, priority zones,
        and waiting between turns.

        Returns:
            A list of zone and turn pairs representing the path.
            Returns an empty list if no path is found.
        """
        stack: List[
            Tuple[float, int, int, str, Zone, Any]
        ] = []

        heapq.heappush(
            stack,
            (
                self.min_dis[self.start_hub],
                0,
                0,
                self.start_hub.name,
                self.start_hub,
        ...
        while stack:
            ...
            for neighbor, connection, cost in neighbors:
                zone_ok = self._can_occupy_zone(
                    neighbor,
                    turn_count + cost,
                )
                conn_ok = self._can_occupy_edge(
                    connection,
                    turn_count + cost,
                )
    ...
    def _can_occupy_zone(
        ...
        Args:
            zone: Zone to check.
            turn: Turn when the drone would occupy the zone.

        Returns:
            True if the zone can be occupied, otherwise False.
        """
        if (
    ...
    def _can_occupy_edge(
        self,
        conn: Connection,
        turn: int,
    ) -> bool:
        """Check whether a connection has available capacity.
        ...
        Returns:
            True if the connection can be used, otherwise False.
        """
        ...
        return (
            ...
            > self.occupied_edges[turn][conn_str]
        )

    @staticmethod
    def _format_connection(conn: Connection) -> str:
        """Create a consistent name for a connection.

        Args:
            conn: Connection to format.

        Returns:
            A connection name containing both zone names.
        """
        con_a: Zone = conn.zone_a
        con_b: Zone = conn.zone_b

        return (
            f"{min(con_a.name, con_b.name)} .. "
            f"{max(con_a.name, con_b.name)}"
        )
    ...
    def find_heuristic(
        ...
        """Calculate the minimum known distance from zones to the end hub.

        Args:
            graph: Graph containing the zones and connections.

        Returns:
            A mapping from each reachable zone to its calculated distance.

        Raises:
            ValueError: If the graph is disconnected.
        """
        pq: List[Tuple[int, str, Zone]] = []
        ...
        while pq:
            ...
            for zone, _, cost_to_b in neighbors:
                new_cost = current_cost + cost_to_b

                if (
                        zone not in distances
                        or new_cost < distances[zone]
                ):
                    distances[zone] = new_cost
                    heapq.heappush(
                        pq,
                        (new_cost, zone.name, zone),
                    )

        if len(distances) != len(graph.hubs):
            raise ValueError("Error: graph is disconnected")

        return distances

    def plan_every_path(
        self,
    ) -> List[List[Tuple[Zone, int]]]:
        """Plan paths for all drones and record their occupied resources.

        Returns:
            A list containing the planned path for each drone.
        """
        ...
        for _ in range(self.nb_drones):
            ...
            for zone, turn in path:
                ...
                # wait
                    ...
                    connect = self.graph.get_connections(
                        zone,
                        previous_step,
                    )
                    previous_step = zone
                    ...
                    if connect is not None:
                        con_str = self._format_connection(
                            connect
                        )
                        ...
                        if (
                            con_str
                            not in self.occupied_edges[turn]
                        ):
                            self.occupied_edges[turn][
                                con_str
                            ] = 1
